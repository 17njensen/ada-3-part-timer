...
procedure part5  is 
	...
	subtype randRange is Integer range 1..5; --1 out of 5 chance to be random
	...
	begin      
		if (F1_begin = True) and FinishF1 - StartF1 = 0.0000 then
		--if StartF1 = 0.0 and then FinishF1 = 0.0 then --if starting to change time
	...
	begin      
		if (F2_begin = True) then
		--if StartF2 = 0.0 and then FinishF2 = 0.0 then --if starting to change time
			Put_Line(""); --Add a new line 
	...
	begin      
		if (F3_begin = True) then
		--if StartF3 = 0.0 and then FinishF3 = 0.0 then --if starting to change time
			Put_Line(""); --Add a new line 
	...
	--Declare Watchdog task
	task Watchdog is
		entry Put(ct: in Time);
	end Watchdog;
	...
	begin
		...
		loop
			select
				accept Put(ct: Time) do
					Put_Line("");
					Put_Line("----F3 missed the deadline----");
					wStart := ct; --time when starting watchdog (variable After)
					Loop
						wAfter := Ada.Calendar.Clock;
						exit when (wAfter - wStart) >= 0.7000000000; --delay until whole number (F1 took 0.3 sec, so delay of 0.7)
					end loop;
				end Put;
			or 
				terminate;
			end select;
		end loop Main; --loop to prevent tasking errors
	end Watchdog;
...
begin   
	...
	--Main loop  
	loop 
		R := Random(gen); --our random variable to create missed deadline
		Finished := False; --flag for F3
		...
		---------------------------------------------------------------F1 starts
			...
			if(R /= 1) then --only run if F3 deadline not missed
		...
		---------------------------------------------------------------F1 ends
			...
			if (R = 1) then --if 1 in 5 chance is true
				--trigger watchdog task event
				Watchdog.Put(After); --needs to take in current F1 time to resync
			end if;
			--******************--
			if(R /= 1) then
			F3_loop:
				loop --loop until 0.5s after F1 starts
					...
					if (F3_after - F3_before >= 0.200000) then --Execute every 0.5s (so 0.2s because of F1's 0.3 time) after F1 starts
						...
						F3_bool := False; --not starting so false
						F3(Currtime => vTime + 0.5, StartF3 => F3_Start, FinishF3 => F3_Curr, F3_begin => F3_bool); --output F3
						Finished := True; --F3 loop is done
					end if;
					exit F3_loop when Finished; 
			end loop F3_loop;
			end if;
			---------------------------------------------F3 ends
			
			---------------------------------------------F2 starts
			if(R /= 1) then --if not our random variable, start F2
